...
class StarvarsModel:
    ...
    def release_points(self, reset=False):
                    ...
                    while cmp(relation_from_particle, new_relations) != 0:
                        ...
                        if self.model.points[j].has_orientation == False:
                            ...
                            if self.model.points[j].orientation == -1 and reset == False:
                                ...
                                particle.orientation = random.gauss(self.model.points[j].orientation,
                                                                    particle.turn_noise)
                            else:
                                pass
                        for l in range(0, len(self.model.points)):
    ...
    def draw_particles(self):
        ...
        for j in range(0, len(self.model.points)):
            for k in range(0, len(self.model.points[j].particles)):
                scaled_x, scaled_y = self.scale(self.model.points[j].particles[k].x,
                                                self.model.points[j].particles[k].y)
                if not self.model.points[j].particles[k].orientation == []:
                    if j == self.driven:
                        self.model.points[j].particles[k].draw_orientation(self.model.points[j].color, scaled_x,
                                                                           scaled_y,
                                                                           self.background)
                    else:
                        self.model.points[j].particles[k].draw_orientation_star(self.model.points[j].color,
                                                                                scaled_x, scaled_y,
                                                                                self.background, 1, self.x_bounds,
                                                                                self.y_bounds)
                else:
                    self.model.points[j].particles[k].draw_without_orientation(self.model.points[j].color, scaled_x,
                                                                               scaled_y,
                                                                               self.background)
        self.display_update()
    ...
    def qualitative_command(self, probabilistic=True):
        ...
        if correct_answers == (len(self.relations)):
    ...
    def sense(self):
        ...
        for i in range(len(driven_agent_data)):
            ...
            self.orientation_driven_agent = max(set(orientation_driven_agent_filter),
                                                key=orientation_driven_agent_filter.count)
        ...
        if self.model is None:
            ...
            for i in range(0, len(self.model.points)):
                for j in range(0, len(self.model.points)):
                    if i != j and j != self.driven and i != self.goal:
                        ...
                        retrieved_data = [i, self.model.points[i].particles[0].orientation, retrieved_relation,
                                          retrieved_relation, j]
    ...
    def move(self, entity, action):
        ...
        for j in range(0, len(changed_particles_index)):
            for k in range(0, len(self.model.points)):
                if entity != k and self.model.points[k].has_orientation is not False:
                    relation.append(self.get_qualitative_relation(self.model.points[k].mean_x,
                                                                  ...
                                                                  self.model.points[entity].particles[
                                                                      changed_particles_index[j]].x,
                                                                  self.model.points[entity].particles[
                                                                      changed_particles_index[j]].y,
            ...
            while cmp(new_relation, relation) == 0 and ctrl <= ctrl_total_per_trial:
                ...
                ## move, and add randomness to the motion command
                dist = float(self.forward)
                self.model.points[entity].particles[changed_particles_index[j]].x = self.model.points[entity].particles[
                                                                                        changed_particles_index[
                                                                                            j]].x + (
                                                                                        cos(rd(
                                                                                            self.model.points[
                                                                                                ...
                                                                                                changed_particles_index[
                                                                                                    j]].orientation)) * dist)
                self.model.points[entity].particles[changed_particles_index[j]].y = self.model.points[entity].particles[
                                                                                        changed_particles_index[
                                                                                            j]].y + (
                                                                                        sin(rd(
                                                                                            self.model.points[
