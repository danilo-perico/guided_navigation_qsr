...
class Robot(pygame.sprite.Sprite,Vision):
    def __init__(self, x, y, theta, KEY, color, vision_dist, fov):
        ...
        self.errors_on = True # Turn errors on and off!
    ...
    def draw_robot(self,robot_index, screen):
        ...
        #pygame.draw.circle(screen.background,self.color,(int(self.x),int(self.y)), 500, 1)
    ...
    def get_orientation(self):
        if self.errors_on:
            ...
            #print 'self.orientation_error, error', self.orientation_error, error
            #print 'self.rotate', self.rotate

        #### for starting in 0 from the orientation of the beginning:
    ...
    def draw_vision(self,screen):
        ...
        startRad = radians(-(degrees(field_of_view)/2) + self.view_rot)
        endRad = radians((degrees(field_of_view)/2) + self.view_rot)
        ...
        pygame.draw.arc(screen.background, self.color,[self.x - vision_dist, self.y - vision_dist, vision_dist * 2, vision_dist * 2], startRad,endRad, 1)
        pygame.draw.arc(screen.background, self.color, [self.x - vision_dist/2, self.y - vision_dist/2, vision_dist, vision_dist], startRad, endRad, 1)

        pygame.draw.line(vision_surface, self.color, vision_surface_center, point_1, 1)
        pygame.draw.line(vision_surface, self.color, vision_surface_center, point_2, 1)
    ...
    def vision_process(self,ballX,ballY,robots):
        ...
        if (self.bkb.read_int(self.Mem, 'DECISION_ACTION_VISION') == 2):  # to test the code, please set 0.
            ...
            # robot detect
            if len(robots) >= 4:
                #print 'LOCALIZATION_FIND_ROBOT', self.bkb.read_int(self.Mem, 'LOCALIZATION_FIND_ROBOT')
                if self.bkb.read_int(self.Mem, 'LOCALIZATION_FIND_ROBOT') == 1:
                    view_rot_aux = self.robot_detect(self.Mem, self.bkb, self.view_rot, self.rotate, self.x, self.y,
                                                     robots[0].x,
                                                     robots[0].y, 0, self.index, self.field_of_view)

                elif self.bkb.read_int(self.Mem, 'LOCALIZATION_FIND_ROBOT') == 2:
                    view_rot_aux = self.robot_detect(self.Mem, self.bkb, self.view_rot, self.rotate, self.x, self.y,
                                                     robots[1].x,
                                                     robots[1].y, 1, self.index, self.field_of_view)

                elif self.bkb.read_int(self.Mem, 'LOCALIZATION_FIND_ROBOT') == 3:
                    view_rot_aux = self.robot_detect(self.Mem, self.bkb, self.view_rot, self.rotate, self.x, self.y,
                                                     robots[2].x,
                                                     robots[2].y, 2, self.index, self.field_of_view)

                elif self.bkb.read_int(self.Mem, 'LOCALIZATION_FIND_ROBOT') == 4:
                    view_rot_aux = self.robot_detect(self.Mem, self.bkb, self.view_rot, self.rotate, self.x, self.y,
                                                     robots[3].x,
                                                     robots[3].y, 3, self.index, self.field_of_view)
                    ...
                    if robots:
                        for j in range(0, len(robots)):
                            if j != self.index - 1 and self.color != robots[j].color:
                                ...
                                self.robot_detect(self.Mem, self.bkb, self.view_rot, self.rotate, self.x, self.y,
                                                  robots[j].x, robots[j].y, j, self.index, self.field_of_view)
    ...
    def draw_starvars(self,screen):
        ...
        resolution = 180 / (m/2)
        half_resolution = resolution / 2
        farthest_boundary = 1500
        # qualitative direction
        pygame.draw.line(screen.background, self.color, (int(self.x), int(self.y)), (
            cos(radians(self.rotate)) * farthest_boundary + int(self.x), int(self.y) -
            sin(radians(self.rotate)) * farthest_boundary), 2)
        for i in range(resolution, 360, resolution):
            pygame.draw.line(screen.background, self.color, (int(self.x), int(self.y)), (
                cos(radians(self.rotate + i)) * farthest_boundary + int(self.x), int(self.y) -
                sin(radians(self.rotate + i)) * farthest_boundary), 1)
        for i in range(half_resolution, 360, resolution):
            self.draw_dashed_line(screen.background, self.color, (int(self.x), int(self.y)), (
               cos(radians(self.rotate + i)) * farthest_boundary + int(self.x), int(self.y) -
               sin(radians(self.rotate + i)) * farthest_boundary), 1)
